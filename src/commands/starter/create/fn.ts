...
export async function ask(): Promise<Create.Answers> {
  const questions = questionFactory<Create.Answers>(question => [
    ...
    question.list('runInstall', {
      message: 'Would you like to install the dependencies right now?',
      choices: [
        { name: 'No', value: false },
        { name: 'Yes', value: true },
      ],
...
export async function createFrondEndProject(
  ...
  const tasks = [
    ...
    new Promise<void>((resolve, reject) => {
      ...
      const filepath = path.resolve(projectPath, '.npmrc');
      try {
        fs.createFileSync(filepath);
        fs.writeFileSync(filepath, content);
        resolve();
    ...
    // .lintstagedrc.json
    new Promise<void>((resolve, reject) => {
      const content = tpl.lintstagedrcJson;
      const filepath = path.resolve(projectPath, '.lintstagedrc.json');
      try {
        fs.createFileSync(filepath);
        fs.writeJsonSync(filepath, content, { spaces: 2 });
        resolve();
